...
import com.yonyou.iuap.purpayment.service.PurpaymentAssoService;
...
/**
 * 说明：采购付款单 主子表Controller——提供主子数据的查询、保存等rest接口
 * 
 * @date 2019-8-28 16:46:53
 */
@Api(value="采购付款单接口",description = "采购付款单主子表Controller——提供主子数据的查询、保存等rest接口")
@Controller("com.yonyou.iuap.purpayment.controller.PurpaymentAssoController")
@RequestMapping(value = "/purpayment/purpayment")
public class PurpaymentAssoController extends BaseController {

    private Logger log = LoggerFactory.getLogger(PurpaymentAssoController.class);
    private PurpaymentAssoService purpaymentAssoService;
    /**
     * 注入主子表service
     */
    @Autowired
    public void setService(PurpaymentAssoService service) {
        this.purpaymentAssoService = service;
    }
     ...
     */
    @ApiOperation(value = "主子表联合查询",httpMethod = "GET",tags = "READ", response = Purpayment.class)
    ...
    public Object  getAssoVo(PageRequest pageRequest,
        ...
        GenericAssoVo vo = purpaymentAssoService.getAssoVo(pageRequest, searchParams);
     ...
     * @return 主表的业务实体
     */
    @ApiOperation( value = "主子表联合保存",httpMethod = "POST",tags = "CREATE", response = Purpayment.class)
    @RequestMapping(value = "/saveAssoVo")
    @ResponseBody
    public Object  saveAssoVo(@RequestBody GenericAssoVo<Purpayment> vo){
        ...
        Object result =purpaymentAssoService.saveAssoVo(vo,annotation);
        return this.buildSuccess(result) ;
    }

    /**
     * 主子表合并处理--主表单条删除,若取消级联删除请在主实体上注解改为@Associative(fkName = "paymentid",deleteCascade = false)
     * @see GenericAssoController
     * @param entities 待删除业务实体
     * @return 删除成功的实体
     */
    @ApiOperation( value = "主子表级联删除",httpMethod = "POST",tags = "DELETE", response = Purpayment.class)
    @RequestMapping(value = "/deleAssoVo")
    @ResponseBody
    public Object  deleAssoVo(@RequestBody Purpayment... entities){
        ...
        if (annotation != null && !StringUtils.isEmpty(annotation.fkName())) {
            int result =0;
            for (Purpayment entity:entities){
                if (StringUtils.isEmpty(entity.getId())) {
                    ...
                    result += this.purpaymentAssoService.deleAssoVo(entity, annotation);
                }
            }
